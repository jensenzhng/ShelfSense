    ...
    try {
        ...
        if (!ingredients || !numberOfRecipes) {
            return res
                .status(400)
                .json({
                    error: "Ingredients and numberOfRecipes are required",
                });
        }
...
app.post("/pantry", async (req, res) => {
    try {
        ...
        if (!userId || !foodItems) {
            return res
                .status(400)
                .json({ error: "userId and foodItems are required" });
        }
...
app.delete("/pantry", async (req, res) => {
    try {
        ...
        if (!userId || !foodItemName) {
            return res
                .status(400)
                .json({ error: "userId and foodItemName are required" });
        }
...
app.post("/editPantry", async (req, res) => {
    try {
        const { userId, foodName, updatedItem } = req.body;

        // Validate input
        if (!userId || !foodName || !updatedItem) {
            return res
                .status(400)
                .json({
                    error: "userId, foodName, and updatedItem are required",
                });
        }

        // Call the updateFoodItem function and pass the necessary parameters
        await updateFoodItem(userId, foodName, updatedItem, req.db); // Assuming req.db is the database connection
